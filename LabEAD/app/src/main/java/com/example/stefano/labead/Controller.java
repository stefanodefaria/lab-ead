...
public class Controller {
    ...
    private static Date timeOutDate;
    ...
    public static void iniciarOperacaoRede(Context context){
        ...
        try{
            ...
            Operation loginOp = OperationsBuilder.loginOperation("student1@test.com", "12345");

            new HttpsOperation(context, loginOp);
        }
        catch (Exception e){
    ...
    public static void receberJson(ArrayList<String> error, Operation operation){
        if(error.size()>0) {
            ...
            try{
                resp = operation.getResponse().getString("token");
            }
            catch (JSONException e){
                resp = "erro";
            }

            Toast.makeText(telaGravidade, resp, Toast.LENGTH_SHORT).show();
        }
    }
